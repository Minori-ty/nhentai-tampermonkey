...
export function createInfiniteScroll(selector: string) {
    ...
    const totalElement = document.querySelector<HTMLAnchorElement>(
        '.pagination > .last',
    );
    if (totalElement) {
    ...
    const pagination =
        document.querySelector<HTMLElement>('section.pagination');
    if (pagination) {
        pagination.remove();
    }
        ...
        (entries: IntersectionObserverEntry[]) => {
            entries.forEach((entry) => {
                if (entry.isIntersecting) {
                    ...
                    const maxPage = [...map.entries()].find(
                        ([_, value]) => value === maxCount,
                    )![0];
    ...
    if (container) {
        container
            .querySelectorAll<HTMLImageElement>('img.lazyload')
            .forEach((img) => {
