...
export async function singlePage() {
    ...
    if (!content) return
    const numPagesElement = document.querySelector<HTMLDivElement>('.page-number.btn.btn-unstyled > .num-pages')
    ...
    async function loadImages() {
        ...
        while (requestPool.length > 0 || inProgress.length > 0) {
            if (inProgress.length < maxConcurrentRequests && requestPool.length > 0) {
                ...
                const imagePromise = loadImage(imageUrl, imageIndex)
                    .then(({ page, img }) => {
                        const imgWrapper = document.getElementById(`image-wrapper-${page}`)
                        if (!imgWrapper) return
                        const placeholder = imgWrapper.querySelector<HTMLDivElement>('.image-placeholder')
                        if (placeholder) {
                            placeholder.remove()
                        }
                    ...
                    .catch(({ index }) => {
                        ...
                        requestPool.push(imageUrls[index]) // 按顺序将失败的图片重新放回池中
                    })
                    .finally(() => {
                        // 加载完成，从进行中的任务中移除
...
// 加载图片
function loadImage(imageUrl: string, index: number): Promise<{ page: number; img: HTMLImageElement }> {
